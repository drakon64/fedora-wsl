<#
.SYNOPSIS
A script to port Fedora 34 to WSL
.DESCRIPTION
A simple PowerShell script to port Fedora 34 to WSL
.PARAMETER Distribution
What the distro will be called in WSL, by default it will be called Fedora-34
.PARAMETER Path
...
Param (
	[String] $Distribution = "Fedora-34",
	[System.IO.DirectoryInfo] $Path = "C:\WSL\",
	[Switch] $SetDefault,
	[Switch] $Wslu
)

Invoke-WebRequest -Uri https://dl.fedoraproject.org/pub/fedora/linux/releases/34/Container/x86_64/images/Fedora-Container-Base-34-1.2.x86_64.tar.xz -OutFile Fedora-Container-Base-34-1.2.x86_64.tar.xz

If (Get-Command -Name xz -CommandType Application) {
	xz --decompress --force .\Fedora-Container-Base-34-1.2.x86_64.tar.xz
}
Else {
	...
	Start-Process -FilePath $Xz -ArgumentList "--decompress --force .\Fedora-Container-Base-34-1.2.x86_64.tar.xz" -NoNewWindow -Wait
	$XzZip, $Xz | Remove-Item
}
tar tf .\Fedora-Container-Base-34-1.2.x86_64.tar | Where-Object { $_ -Like "*/layer.tar" } | ForEach-Object { tar xf .\Fedora-Container-Base-34-1.2.x86_64.tar --strip-components=1 "$_" }
...
wsl --distribution "$Distribution" --exec dnf -y --releasever 34 install shadow-utils passwd cracklib-dicts sudo generic-logos generic-release generic-release-notes
If ($Wslu) {
	wsl --distribution "$Distribution" --exec dnf -y copr enable wslutilities/wslu fedora-34-x86_64
